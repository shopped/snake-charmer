...
from Realtime_PyAudio_FFT.fft import getFFT
from Realtime_PyAudio_FFT.utils import *
...
class Stream_Analyzer:
    ...
    def __init__(self,
        ...
        try:
            from Realtime_PyAudio_FFT.stream_reader_pyaudio import Stream_Reader
            ...
            from Realtime_PyAudio_FFT.stream_reader_sounddevice import Stream_Reader
            self.stream_reader = Stream_Reader(
                device  = device,
                rate    = rate,
                updates_per_second  = updates_per_second,
        ...
        if self.visualize:
            from Realtime_PyAudio_FFT.visualizer import Spectrum_Visualizer
        ...
        # print("Printing fftx frequencies")
        # for x in self.fftx:
        #     print(x)
        # print("Done printing {} frequencies".format(len(self.fftx)))
    ...
    def update_features(self, n_bins = 3):
        ...
        # self.fft_clipped = self.fftx
        self.strongest_frequency = self.fftx[np.argmax(self.fft)]
        self.strongest_volume = np.max(self.fft)
    ...
    def get_audio_features(self):
        ...
        return {"vol": self.strongest_volume,
            "note": frequency_to_note(self.strongest_frequency)}